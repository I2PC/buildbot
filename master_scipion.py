...
setXMIPPCUDA_BIN = ShellCommand(
    command=util.Interpolate(
        'sed -ie "\$aCUDA_BIN = {}" %(prop:SCIPION_LOCAL_CONFIG)s'.format(settings.XMIPP_CUDA_BIN)),
...
def installSDevelScipionFactory(groupId):
    ...
    installXmippCmd =  'rm -rf xmipp-bundle && rm -rf scipion-em-xmipp && git clone https://github.com/I2PC/xmipp3.git xmipp-bundle && ./scipion3 run pip install xmipp3-installer && ./scipion3 run xmipp-bundle/xmipp  &&  cd xmipp-bundle/src && git clone https://github.com/I2PC/scipion-em-xmipp.git && ../../scipion3 installp -p . --devel'
    installScipionFactorySteps.addStep(ScipionCommandStep(
        command=installXmippCmd,
        name='Installing Xmipp',
        description='Installing Xmipp',
    ...
    installCmd = (settings.SCIPION_CMD + ' pip install pyxmipp3')
...
def getScipionBuilders(groupId):
    ...
    if groupId == settings.PROD_GROUP_ID:
        ...
        if groupId == settings.SDEVEL_GROUP_ID:
            ...
            env['XMIPP_HOME'] = settings.SPROD_XMIPP_HOME
